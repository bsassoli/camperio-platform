...
**Deciso il 16/09/2026:** vale **MOV**, cioè la versione di settembre. La motivazione dell'autore rovescia quella del changelog di giugno: MOV intercetta anche **trasferimenti** e **operazioni sul capitale**, che ORD — contenendo solo i trade — non vede, e senza quelle righe la quantità ricostruita alla data AL è sbagliata. Lo stesso vale per i conti con operatività in derivati o con rettifiche (caso in arrivo dall'autore). Il changelog del 26/06 è quindi superato **nella conclusione, non nell'osservazione**: le righe tecniche (collaterale, giroconti fra depositi) esistono e continuano a non riconciliare.

Conseguenza sul back-port: non si prende `MOV.TIPOMO` così com'è, si prende **MOV filtrato per causale** — le causali che spostano la quantità posseduta dentro, quelle tecniche fuori — e l'insieme delle causali va deciso su dati veri, non per elenco a priori.

La quadratura richiesta dall'autore (due conti a una data passata, ORD e MOV a confronto) serve esattamente a questo: **non a scegliere il metodo** — la scelta è fatta — ma a **classificare ogni divergenza**. Attesa: divergenza spiegata da un trasferimento o da un'operazione sul capitale → MOV corretto, ORD sbagliava in silenzio; divergenza spiegata da una riga tecnica → causale da escludere dal filtro. Da produrre per ogni conto: quantità per titolo con i due metodi alla stessa data, delta, e per ogni delta le righe MOV che lo generano con la loro causale.

Nello stesso passaggio va allineato `price_changes` e corretta la docstring di `comitato_extra`, altrimenti il monorepo resta misto come la NUOVA.
...
| 9 | `MOV.TIPOMO`+`CTVTIT` **filtrato per causale** al posto di `ORD.TIPOPE` per le quantità alla data AL: MOV intercetta trasferimenti e operazioni sul capitale che ORD non vede, le righe tecniche restano fuori. Deciso il 16/09, **subordinato alla quadratura** ORD vs MOV su due conti a una data passata (che serve a fissare il filtro, non a scegliere il metodo); include l'allineamento di `price_changes`, che usa ancora ORD | `data_layer.py` | media, **dopo** la quadratura |
...
- **`ORD` o `MOV`** per le quantità alla data AL → **vale `MOV`** (la versione di settembre), perché intercetta anche **trasferimenti** e **operazioni sul capitale**: ORD contiene solo i trade, quindi salta proprio le righe che spostano la quantità senza un ordine dietro. Lo stesso sui conti con derivati o rettifiche; l'autore manda il caso. Il changelog del 26/06 è superato **nella conclusione, non nell'osservazione**: le righe tecniche (collaterale, giroconti) esistono, quindi si porta **MOV filtrato per causale**, non MOV grezzo. **Condizione posta dall'autore:** prima di chiudere, quadratura su un paio di conti a una data passata con i **due metodi a confronto**, per classificare ogni divergenza (trasferimento/capitale → MOV giusto; riga tecnica → causale da escludere). Da allineare nello stesso passaggio `price_changes`, che usa ancora ORD (righe 370-371) — la versione NUOVA resta mista. Vedi §3.2 e voce 9.
